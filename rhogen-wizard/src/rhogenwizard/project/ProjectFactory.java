...
public class ProjectFactory implements IProjectFactory {
	...
	private IProject createBaseProject(BuildInfoHolder projectInfo) throws AlredyCreatedException, CoreException {
		...
		IProject newProject = ResourcesPlugin.getWorkspace().getRoot()
				.getProject(projectInfo.appName);
	...
	private IProject createBaseProject(rhogenwizard.wizards.rhosync.BuildInfoHolder projectInfo)	throws AlredyCreatedException, CoreException {
		// it is acceptable to use the ResourcesPlugin class
		IProject newProject = ResourcesPlugin.getWorkspace().getRoot()
				.getProject(projectInfo.appName);
		...
		if (!newProject.exists()) {
			...
			if (true) {//por ahora solo se trabajara en el work space
				path = getWorkspaceDir() + File.separator + projectInfo.appName;
			} else {
				path = projectInfo.appDir;
	...
	private IRhomobileProject createRhomobileProject(
			Class<? extends IRhomobileProject> projectTag, IProject project)
			throws BadProjectTagException {
		...
		} else if (projectTag.equals(RhoSyncProject.class)) {
			return new RhoSyncProject(project);
		}
	...
	public IRhomobileProject createProject(
			Class<? extends IRhomobileProject> projectTag,
			BuildInfoHolder projectInfo) throws CoreException,
			ProjectNotFoundException, AlredyCreatedException,
			BadProjectTagException {
		...
		IRhomobileProject rhoProject = createRhomobileProject(projectTag,
				project);
	...
	public IRhomobileProject createProject(Class<? extends IRhomobileProject> projectTag, rhogenwizard.wizards.rhosync.BuildInfoHolder projectInfo)
	...
	@Override
	public Class<?> typeFromProject(IProject project)
			throws BadProjectTagException {
		if (RhodesProject.checkNature(project)) {
